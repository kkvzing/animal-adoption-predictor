...
# Ensure working directory is always the app/ folder regardless of how the app is launched
...
def load_artifacts():
    ...
    reg_preprocessor = joblib.load("../artifacts/regression_preprocessor.joblib")
    reg_model = joblib.load("../artifacts/regression_best_model.joblib")

    with open("../artifacts/feature_names.json") as f:
        feature_names = json.load(f)

    feat_imp = pd.read_csv("../artifacts/feature_importances.csv", index_col=0)

    adoption_by_type = pd.read_csv("../artifacts/adoption_by_type.csv")
    adoption_by_condition = pd.read_csv("../artifacts/adoption_by_condition.csv")
    ...
    return (
        ...
        feat_imp, adoption_by_type, adoption_by_condition, metadata
    )

(model, preprocessor,
 reg_preprocessor, reg_model, feature_names,
 feat_imp, adoption_by_type, adoption_by_condition, metadata) = load_artifacts()
...
— such as neutering the animal or treating a medical condition — might affect the outlook.
    ...
    """, unsafe_allow_html=True)
        ...
        st.image("assets/line1.png", use_container_width=True)
...
# ============================================================
    ...
    def load_overview_data():
        df = pd.read_csv("../artifacts/X_train_raw.csv")
        y = pd.read_csv("../artifacts/y_train.csv").values.ravel()
        df['adopted'] = y
        return df

    @st.cache_data
    def load_regression_data():
        return pd.read_csv("../artifacts/cleaned_data_regression.csv")
    ...
    with st.container(border=False, key="overview_card_2"):
        ...
        <div style="margin-bottom: 0.6rem;"></div>
        <div class="overview-section-note">
            Select an animal type below to see
    ...
    st.markdown("   ")
    st.image("assets/line2.png", use_container_width=True)
    st.markdown("   ")
